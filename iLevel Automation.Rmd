...
# User and Contact Detail
user_contact_detail <- sf_query("SELECT Id,
                                    ManagerId,
                                    Manager.Name,
                                    (SELECT
                                      Id,
                                      ...
                                      pse__Region__r.Name,
                                      pse__Group__r.Name
                                    FROM pse__Contacts__r)
                                  FROM User
                                  WHERE Id IN (SELECT pse__Salesforce_User__c FROM Contact WHERE pse__Is_Resource_Active__c = TRUE)")
...
milestones_monthly_hours <- sf_query(paste0("SELECT Milestone__c,
                                              ...
                                              WHERE (CreatedDate >=", from_date, "T00:00:00.000Z
                                                OR (Calculated_For_Date__c >=", from_date, "
                                              AND Calculated_For_Date__c <=", to_date, "))
...
issues <- issues %>% 
  ...
  left_join(user_contact_detail, 
            by=c("Issue_Owner__c" = "Id")) %>% 
...
opportunities_with_products <- opportunities_with_products %>% 
  ...
  left_join(user_contact_detail, by=c("Services_Referred_by_Users__c" = "Id"))
...
surveys <- surveys %>% 
  left_join(user_contact_detail, by=c("Case__r.Owner__c" = "Id")) %>% 
...
milestones_with_monthly_hours <- milestones_monthly_hours %>% 
  ...
  mutate(Last_Month = Calculated_For_Date__c %>% floor_date("months"),
         Month_Calculated_For__c = Last_Month %m+% months(1)
         ) %>% 
  ...
  filter(is.na(Month_Actual_Go_Live__c) | 
           Last_Month <= (Month_Actual_Go_Live__c %m+% months(2)),
...
ds_work_tasks <- ds_work_tasks %>% 
  left_join(user_contact_detail, by=c("OwnerId" = "Id")) %>% 
...
cases <- cases %>% 
  left_join(user_contact_detail, by=c("Owner__c" = "Id"))
...
dc_cases <- cases %>% 
  ...
  mutate(Product = Product_Family_Picklist__c,
         Product = case_when(Product %in% c("3P Risk Management") ~ "RiskRate",
                             ...
                             Product %in% c("Awareness", "Expolink", "GRC Insights", "NAVEX One", "NG Gateway", NA) ~ "Other",
                             Product %in% c("ESG") ~ "ESG",
...
at_risk_arr_location <- create_metrics(issues,
                                       ...
                                       .filter = Status__c %in% c("Unresolved",
                                        ...
                                        !Product_Family__c %in% c("Lockpath", "IRM"),
                                       c(`ARR at Risk` = AccountName__r.Total_Asset_ARR__c),
                                       .groups = c(Location = Contact.pse__Region__r.Name, 
                                                   Risk = Issue_Type__c)
                                       )
...
at_risk_issues_opened_location <- create_metrics(issues, 
                                                 ...
                                                 .filter = Date_Reported__c >= from_date &
                                                   ...
                                                   !Product_Family__c %in% c("Lockpath", "IRM"), 
                                                 .groups = c(Month = Month_Date_Reported__c,
                                                             Location = Contact.pse__Region__r.Name,
                                                             Risk = Issue_Type__c),
                                                 c(`Issues Opened` = Id))
...
qualified_leads_booked_location <- create_metrics(opportunities %>% 
                                                  ...
                                                  .groups = c(Month = Month_CloseDate,
                                                              Location = Contact.pse__Region__r.Name,
                                                              Team = Contact.pse__Group__r.Name),
                                                  c(Amount))
...
planned_hours_completed_region <- create_metrics(milestones_with_monthly_hours,
                                                 ...
                                                 .groups = c(Group = Group,
                                                             ...
                                                             Region = Account__r.Reporting_Region__c),
                                                 c(`Planned Hours Completed (Last Month)`, 
                                                   Submitted_Hours__c, 
                                                   Submitted_against_Planned__c, 
                                                   Under_Planned_Hours__c, 
                                                   Total_Planned_Hours__c))
