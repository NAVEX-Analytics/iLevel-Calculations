...
customer_satisfaction_region <- create_metrics(surveys,
                                               ...
                                               c(`Professional Services CSAT` = PS_Comp_q3__c, 
                                                 ...
                                                 `Customer Support CSAT` = Q1_Technician_Assist__c, 
...
customer_satisfaction_location_imp_ps <- create_metrics(surveys,
                                                 categories = c(Group = "Customer Satisfaction",
                                                                Metric = "Average Score"),
                                                 .fn = list(average = mean),
                                                 .filter = !Project_Primary_Product__c %in% c("Lockpath", "IRM") &
...
customer_satisfaction_location_support <- create_metrics(surveys,
                                                         categories = c(Group = "Customer Satisfaction",
                                                                        Metric = "Average Score"),
                                                         .fn = list(average = mean),
                                                         .filter = Survey_Completed__c >= from_date &
                                                           Survey_Completed__c <= to_date &
                                                           !is.na(Case__r.Owner__r.Location__c),
                                                         .groups = c(Month = Month_Survey_Completed__c,
...
delivery_efficiency_region <- create_metrics(milestones,
                                             ...
                                             .filter = pse__Project__r.pse__Project_Status__c!="Terminated" &
                                                ...
                                                Actual_Go_Live__c <= to_date &
                                                Group %in% c("Implementation", "Professional Services"),
                                             .groups = c(Month = Month_Actual_Go_Live__c,
                                                         Group = Group,
                                                         Region = Account__r.Reporting_Region__c, 
                                                         Product = Product),
                                             c(`Total Approved Hours` = Total_Approved_Hours__c, 
                                               `Total Planned Hours` = Total_Planned_Hours__c)) %>% 
  group_by(Metric, Month, Group, Region, Product) %>% 
  reframe(`Delivery Efficiency` = mean(`sum of Total Planned Hours` / `sum of Total Approved Hours`))
...
delivery_efficiency_location <- create_metrics(milestones,
                                               ...
                                               c(`Total Approved Hours` = Total_Approved_Hours__c, 
                                               `Total Planned Hours` = Total_Planned_Hours__c)) %>% 
  group_by(Metric, Month, Group, Location, Product) %>% 
  reframe(`Delivery Efficiency` = mean(`sum of Total Planned Hours` / `sum of Total Approved Hours`))
...
location_data <- prepare_output(keep_cols = c(Month, Group, Metric, Location, Calculation, Value),
                                ...
                                customer_satisfaction_location_imp_ps,
                                customer_satisfaction_location_support,
                                nps_region,
