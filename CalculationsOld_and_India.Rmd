...
qualified_leads_query <- paste("SELECT
                    ...
                    WHERE
                      ...
                      AND Include_In_Bookings__c = TRUE
                      AND Product2.Pricing_Type__c = 'Annual'
...
proj_hours_sold_query <- paste0("SELECT
      ...
      WHERE
        ...
        AND ((pse__Project__r.pse__Opportunity__r.CloseDate >= ", ymd(from_date) %m+% months(-2), "
          AND pse__Project__r.pse__Opportunity__r.CloseDate <= ", to_date , ")
        OR (CreatedDate >= ", ymd(from_date) %m+% months(-2), "T00:00:00.000Z 
          AND CreatedDate <= ", to_date, "T23:59:59.999Z 
...
support_cases_opened_query <- paste0("SELECT Id,
                      Client_Care_Team__c,
                      Age_Days__c,
                      Skill__c,
                      CreatedDate,
                      ...
                      WHERE
                             ...
                             AND CreatedDate >= ", from_date, "T00:00:00Z 
                             AND CreatedDate <= ", to_date, "T00:00:00Z
                      ")

support_cases_closed_query <- paste0("SELECT Id,
                      Client_Care_Team__c,
                      Age_Days__c,
                      Skill__c,
                      CreatedDate,
                      ...
                      WHERE
                             ...
                             AND Client_Care_Team__c != 'NetClaim'
                             AND IsClosed = TRUE
                             AND ClosedDate >= ", from_date, "T00:00:00Z 
                             AND ClosedDate <= ", to_date, "T00:00:00Z"
)

disc_cases_o_query <- paste0("SELECT Id,
                      CreatedDate,
                      ClosedDate,
                      Account.Reporting_Region__c,
                      Owner__r.Name,
...
disc_cases_c_query <-paste0("SELECT Id,
                      CreatedDate,
                      ClosedDate,
                      Account.Reporting_Region__c,
                      Owner__r.Name,
                      ...
                      WHERE
                             ...
                             AND (Account.Test_Account__c = FALSE OR Account.Id = '0015000000iMvunAAC')
                             AND ClosedDate >= ", from_date, "T00:00:00Z
                             AND ClosedDate <= ", to_date, "T23:59:59Z"
                     )
...
support_cases_o <- sf_query(support_cases_opened_query)
support_cases_c <- sf_query(support_cases_closed_query)
disc_cases_o <- sf_query(disc_cases_o_query)
disc_cases_c <- sf_query(disc_cases_c_query)
...
issues_closed <- issues_pre %>%
  filter(between(date_resolved, as.Date(from_date), as.Date(to_date)),
        !product %in% "Lockpath",
        status %in% c("Resolved", "Closed - Resolved", "Closed - Unresolved",
                      "Closed - NPS", "Closed - Unresponsive NPS", "Closed - Unresponsive Health Data")) %>% 
  summarize(
    `Region` = "Core",
    `Opened` = n()
  ) %>% 
...
qlb_calc <- qlb_pre %>% 
  ...
  arrange(opportunity_referrer_location)
...
csat_core <- surveys_pre %>%
  filter(!team %in% c("Lockpath", "IRM"),
         !product %in% c("Lockpath", "IRM")) %>% 
  group_by(`AE Segment`) %>% 
  summarize(
  ...
  bind_rows(surveys_pre %>% filter(!team %in% c("Lockpath", "IRM"),
                                   !product %in% c("Lockpath", "IRM")) %>% 
           summarize(
             `AE Segment` = "Total",
             `Customer Support` = round(mean(support_csat, na.rm = TRUE) * 10, 2),
...
csat_us <- surveys_pre %>% 
  filter(location %in% "US"|region %in% "US",
         !team %in% c("Lockpath", "IRM"),
         !product %in% c("Lockpath", "IRM", "WhistleB")) %>% 
  group_by(`AE Segment`) %>% 
  summarize(
    `Customer Support` = round(mean(support_csat, na.rm = TRUE) * 10, 2),
    `Product Satisfaction` = round(mean(product_csat, na.rm = TRUE) * 10, 2),
...
csat_irm <- surveys_pre %>% 
  filter(team %in% c("Lockpath", "IRM")|product %in% c("Lockpath", "IRM")) %>% 
  group_by(`AE Segment`) %>% 
  summarize(
    `Customer Support` = round(mean(support_csat, na.rm = TRUE) * 10, 2),
    `Product Satisfaction` = round(mean(product_csat, na.rm = TRUE) * 10, 2),
...
```{r project_hours}
proj_hours_sold_pre <- proj_hours_sold %>%
  rename(
      milestone_id = Id,
      milestone_name = Name,
      ...
      opportunity_close_date = Opportunity__r.CloseDate,
      created_date = CreatedDate
      ) %>% 
  mutate(
    group = case_when(
      product_gl_group %in% c("Required Setup", "Other Services") ~ "Implementation",
      product_gl_group %in% c("Professional Services") ~ "Professional Services",
      TRUE ~ as.character(product_gl_group))
  )

proj_hours_sold_core <- proj_hours_sold_pre %>% 
  filter(
    !product_family %in% c("Lockpath", "IRM"),
    opportunity_close_date %within% (from_date %--% to_date) | (created_date %within% (from_date %--% to_date) & grepl("Course Swap", milestone_name))
  ) %>% 
  group_by(account_region, group) %>% 
  summarize(
    hours_sold = round(sum(hours_sold, na.rm = TRUE), 2),
    n = n()
  )

proj_hours_sold_na <- proj_hours_sold_pre %>% 
  filter(
...
proj_hours_sold_eu <- proj_hours_sold_pre %>% 
  filter(
...
proj_hours_sold_india <- proj_hours_sold_pre %>% 
  filter(
...
proj_hours_backlog_on_hold_us <- proj_hours_backlog_pre %>% 
  filter(milestone_status %in% "Delayed On-Hold",
         grepl("AMER", milestone_pm_region)) %>% 
  group_by(group, product) %>% 
  summarize(
    backlog_on_hold_hours = round(sum(backlog_hours, na.rm = TRUE), 2),
    hours_completed = round(sum(hours_completed, na.rm = TRUE), 2),
...
support_cases_o_pre <- support_cases_o %>% 
  rename(
    ...
    case_age = Age_Days__c,
     ...
     mutate(Month = floor_date(close_date, unit="month"),
            Product = case_when(
              grepl("EthicsPoint|Telephony", product_team) ~ "EthicsPoint",
              grepl("PolicyTech", product_team) ~ "PolicyTech",
              grepl(" Training", product_team)~ "NAVEXEngage",
              ...
              grepl("Lockpath|IRM", product_team) ~ "Lockpath",
              product_team == TRUE ~ "Other"
              )
     )
support_cases_c_pre <- support_cases_c %>% 
  rename(
    product_team = Client_Care_Team__c,
    close_date = ClosedDate,
    opened_date = CreatedDate,
...
cases_opened <- support_cases_o_pre %>% 
  filter(opened_date %within% (from_date %--% to_date)) %>% 
  mutate(reporting_region = case_when(Product %in% "Lockpath" ~ "Lockpath (excl. from Total)",
  ...
  left_join(
    support_cases_c_pre %>% 
      filter(close_date %within% (from_date %--% to_date)) %>%
      mutate(reporting_region = case_when(Product %in% "Lockpath" ~ "Lockpath (excl. from Total)",
                                      TRUE ~ reporting_region)) %>% 
      group_by(reporting_region, Product) %>% 
      summarize(
      ...
      ), by=c("reporting_region", "Product")) %>% 
  bind_rows(
    bind_cols(
      support_cases_o_pre %>%
        filter(opened_date %within% (from_date %--% to_date),
               !Product %in% "Lockpath") %>% 
        mutate(Product = "Grand Total",
               reporting_region = "Grand Total") %>% 
        group_by(reporting_region, Product) %>%
        summarize(
          cases_opened = n()
      )
...
cases_opened_india <- support_cases_o_pre %>% 
  filter(opened_date %within% (from_date %--% to_date),
         grepl("India", location)) %>%
  group_by(Product) %>% 
  summarize(
    cases_opened = n()
  )
...
cases_backlog_pre <- cases_backlog %>% 
  rename(
    product_team = Client_Care_Team__c,
    opened_date = CreatedDate,
    type = Record_Type__c,
    ...
    case_age = Age_Days__c
     ...
     mutate(Month = floor_date(opened_date, unit="month"),
            Product = case_when(
              grepl("EthicsPoint|Telephony", product_team) ~ "EthicsPoint",
              grepl("PolicyTech", product_team) ~ "PolicyTech",
              grepl(" Training", product_team)~ "NAVEXEngage",
...
cases_backlog_2d <- cases_backlog_pre %>% 
  filter(case_age >= 2) %>% 
  group_by(Product) %>% 
  summarize(
    count = n(),
    `% Total` = round(count/count(cases_backlog_pre %>% filter(case_age >= 2))*100, 2)$n
  ) %>% arrange(desc(count))
  
cases_backlog_10d <- cases_backlog_pre %>% 
  filter(case_age >= 10) %>% 
  group_by(Product) %>% 
  summarize(
    count = n(),
    `% Total` = round(count/count(cases_backlog_pre %>% filter(case_age >= 10))*100, 2)$n
  ) %>% arrange(desc(count))
  
cases_backlog_30d <- cases_backlog_pre %>% 
  filter(case_age >= 30) %>% 
  group_by(Product) %>% 
  summarize(
    count = n(),
    `% Total` = round(count/count(cases_backlog_pre %>% filter(case_age >= 30))*100, 2)$n
  ) %>% arrange(desc(count))
